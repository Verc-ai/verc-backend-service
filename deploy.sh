...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --set-env-vars "NODE_ENV=production,ENVIRONMENT=staging,GCP_PROJECT_ID=${PROJECT_ID},GCP_REGION=${REGION},GCP_TASK_QUEUE_NAME=transcription-queue-staging-v2,CLOUD_TASKS_ENABLED=true,CLOUD_TASKS_SERVICE_ACCOUNT_EMAIL=cloud-tasks-invoker@${PROJECT_ID}.iam.gserviceaccount.com,ASSEMBLYAI_PII_REDACTION_ENABLED=true,ASSEMBLYAI_PII_SUBSTITUTION=entity_name,ASSEMBLYAI_GENERATE_REDACTED_AUDIO=true,MODAL_ENABLED=true,MODAL_APP_NAME=nvidia_canary_qwen,DJANGO_ENV=production,CLOUD_RUN_SERVICE_URL=${SERVICE_URL}" \
  --set-secrets "SUPABASE_URL=supabase-url:latest,SUPABASE_ANON_KEY=supabase-anon-key:latest,SUPABASE_SERVICE_ROLE_KEY=supabase-service-role-key:latest,OPENAI_API_KEY=openai-api-key:latest,ANTHROPIC_API_KEY=anthropic-api-key:latest,ASSEMBLYAI_API_KEY=assemblyai-api-key:latest,MODAL_TOKEN_ID=MODAL_TOKEN_ID:latest,MODAL_TOKEN_SECRET=MODAL_TOKEN_SECRET:latest,TWILIO_ACCOUNT_SID=twilio-account-sid:latest,TWILIO_AUTH_TOKEN=twilio-auth-token:latest,TWILIO_PHONE_NUMBER=twilio-phone-number:latest,TWILIO_WEBHOOK_BASE_URL=TWILIO_WEBHOOK_BASE_URL:latest,BUFFALO_PBX_USERNAME=BUFFALO_PBX_USERNAME:latest,BUFFALO_PBX_PASSWORD=BUFFALO_PBX_PASSWORD:latest,BUFFALO_SIP_USERNAME=BUFFALO_SIP_USERNAME:latest,BUFFALO_SIP_PASSWORD=BUFFALO_SIP_PASSWORD:latest" \
  --memory 1Gi \
  --cpu 1 \
  --min-instances 0 \
  --max-instances 5 \
