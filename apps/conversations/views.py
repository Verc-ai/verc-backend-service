...
class UploadView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.info(f'Received audio file upload: {audio_file.name} ({audio_file.size} bytes)')
            ...
            try:
                ...
                logger.error(f'Failed to upload to Supabase Storage: {e}', exc_info=True)
                return Response(
            ...
            try:
                ...
                logger.info(f'✅ Created transcription session {session_id} in Supabase')
            except Exception as e:
                logger.error(f'Failed to create session in Supabase: {e}', exc_info=True)
            ...
            print(f'[UPLOAD] Cloud Tasks config: enabled={cloud_tasks_config.enabled}, project_id={cloud_tasks_config.project_id}, region={cloud_tasks_config.region}, queue_name={cloud_tasks_config.queue_name}', file=sys.stderr, flush=True)
            logger.info(
            ...
            if cloud_tasks_config.enabled:
                try:
                    from apps.core.services.cloud_tasks import enqueue_transcription_task, get_cloud_tasks_client
                    
                    # Test if client can be created
                    test_client = get_cloud_tasks_client()
                    if not test_client:
                        logger.error('❌ Cloud Tasks client creation failed - cannot queue task')
                    else:
                    ...
                    if not service_url:
                        ...
                        if k_service:
                            ...
                            logger.info(f'Using request host as Cloud Run service URL: {service_url}')
                        else:
                            service_url = request.build_absolute_uri('/').rstrip('/')
                            logger.warning(
                    ...
                    print(f'[UPLOAD] Attempting to queue transcription task: sessionId={session_id}, storagePath={storage_path}, serviceUrl={service_url}', file=sys.stderr, flush=True)
                    logger.info(f'🔵 Attempting to queue transcription task: sessionId={session_id}, storagePath={storage_path}, serviceUrl={service_url}')
                    ...
                    print(f'[UPLOAD] Task queued result: {task_queued}', file=sys.stderr, flush=True)
                    if task_queued:
                        print(f'[UPLOAD] ✅ Transcription task queued successfully for session {session_id}', file=sys.stderr, flush=True)
                        logger.info(f'✅ Transcription task queued successfully for session {session_id}')
                        return Response({
                            'sessionId': session_id,
                            'storagePath': storage_path,
                            'audioUrl': signed_url or f'https://{config.url.replace("https://", "").split(".")[0]}.supabase.co/storage/v1/object/public/{bucket}/{storage_path}',
                            'originalName': audio_file.name,
                    ...
                    logger.error(f'❌ Exception while queuing Cloud Task: {e}', exc_info=True)
                    # Continue and return 200 - upload succeeded even if task queueing failed
                    return Response({
                        'sessionId': session_id,
                        'storagePath': storage_path,
                ...
                logger.warning('Cloud Tasks is disabled - not queuing transcription task')
            
            # If Cloud Tasks not enabled, return 200 with sessionId
            return Response({
                'sessionId': session_id,
                'storagePath': storage_path,
                'audioUrl': signed_url or f'https://{config.url.replace("https://", "").split(".")[0]}.supabase.co/storage/v1/object/public/{bucket}/{storage_path}',
    ...
    def post(self, request):
        try:
            ...
            if supabase:
                ...
                logger.warning('Supabase not available, session not persisted to database')
            ...
            # If Cloud Tasks not enabled, return session ID for manual processing
            return Response({
                'simulationId': session_id,  # Frontend expects this as the session ID
                'message': 'Simulation started (session created)'
            }, status=status.HTTP_200_OK)
