...
class SessionListView(APIView):
    ...
    def get(self, request):
        ...
        if not supabase:
            ...
            logger.warning(
                f"Supabase config - URL: {bool(config.url)}, Service Key: {bool(config.service_role_key)}"
            )
            logger.warning(
                f'Supabase config details - URL length: {len(config.url) if config.url else 0}, Key length: {len(config.service_role_key) if config.service_role_key else 0}, URL empty: {not config.url or config.url.strip() == ""}, Key empty: {not config.service_role_key or config.service_role_key.strip() == ""}'
            )
            return Response(
                {"sessions": [], "total": 0, "debug": "Supabase client not available"},
        ...
        try:
            ...
            if len(response.data) == 0:
                logger.info(
                    "No sessions found in database. Checking if table exists and has data..."
                )
            ...
            if session_ids:
                try:
                    ...
                    events_response = (
                        supabase.table("transcription_events")
                        .select("session_id")
            ...
            for row in response.data:
                ...
                # Determine final status based on AI completion (override database status)
                # Check if AI analysis is complete
                if (
                    row.get("call_summary_status") == "completed"
                    and row.get("call_scorecard_status") == "completed"
                ):
                    session_status = "completed"
                elif (
                    row.get("call_summary_status") == "in_progress"
                    or row.get("call_scorecard_status") == "in_progress"
                ):
                    session_status = "analyzing"
                elif row.get("last_event_received_at"):
                    # Has transcription events but AI not started/complete
            ...
            return Response(
                {
                    "sessions": sessions,
                    "total": (
                        response.count if hasattr(response, "count") else len(sessions)
                    ),
                    "limit": limit,
                    "offset": offset,
                },
...
class SessionDetailView(APIView):
    ...
    def get(self, request, session_id):
        ...
        try:
            ...
            logger.info(
                f'Session found: {session_id}, status={session_data.get("status")}'
            )

            # Fetch all transcription events for this session
            events_response = (
                supabase.table(config.events_table)
                .select("*")
            ...
            logger.info(
                f"Found {len(events)} transcription events for session {session_id}"
            )
            ...
            if audio_storage_path:
                try:
                    ...
                    if hasattr(signed_url_response, "data") and isinstance(
                    ...
                    if audio_url:
                        ...
                        logger.warning(
                            f"Signed URL response format unexpected: {type(signed_url_response)}"
                        )
                except Exception as e:
            ...
            response_data = {
                ...
                # Calculate duration if possible (convert ms to seconds)
                "duration": (
                    int(metadata.get("duration") / 1000)
                    if metadata.get("duration")
                    else self._calculate_duration(
            ...
            logger.info(
                f"Returning session details: {len(transcription)} events, audio_url={bool(audio_url)}"
            )
